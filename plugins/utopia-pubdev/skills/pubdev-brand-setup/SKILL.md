...
description: >
  ...
  docs/pubdev-brand.md for the utopia-pubdev README standard to consume.
  Applies when a non-Utopia Dart/Flutter repo adopts the README standard, when
  the utopia-pubdev skill finds no profile, or when an existing profile needs
  changing. NOT needed in Utopia codebases (the bundled Utopia profile
  auto-applies) and NOT for composing the README itself (that's utopia-pubdev).
---

# pubdev-brand-setup - branding profile interview

One-time per repo: produce `docs/pubdev-brand.md`, the brand profile the
`utopia-pubdev` README standard reads. Schema, field semantics, and the file
template live in
[brand-profile.md](../utopia-pubdev/references/brand-profile.md) - this skill
is the flow that fills it in.
...
1. **Check what already applies.**
   ...
   - The repo is detected as Utopia's (rule in
     [brand-profile.md](../utopia-pubdev/references/brand-profile.md#resolution-order))
...
4. **Confirm once, then write.** Echo the full assembled profile (derived +
   ...
   the template in
   [brand-profile.md](../utopia-pubdev/references/brand-profile.md#file-template).
   Committing stays with the user.

5. **Hand off.** README composition itself is the `utopia-pubdev` skill - the
   freshly written profile now resolves first, so it picks the branding up
   automatically.
