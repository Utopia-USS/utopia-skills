...
for each wave in phase_a_waves:
  ...
  3. SERIAL commit loop - for each item in filtered-wave order:
     ...
     c. Invoke utopia-hooks-migrate-bloc:migrate-review
     ...
     e. Refresh MIGRATION.md (before committing):
        - Invoke utopia-hooks-migrate-bloc:migrate-inventory (read-only, fast).
...
After Phase A: `migrate-inventory` already re-ran after each successful commit in step 3e. Orchestrator uses the last `migrate-inventory` output + its in-memory `blocked` set for Phase B work_list without a separate re-run here.
...
while work_list not empty and budget_remaining > 0:
  ...
  for each screen in batch (parallel Agent calls in one message):
    ...
    4a. Refresh MIGRATION.md (before committing):
        ...
        - The commit includes: all files_touched by migrate-screen + MIGRATION.md refresh.

  # The last migrate-inventory run (from step 4a of the final screen in the batch) already
  # has fresh work_list input. No separate inter-batch run needed.
```
...
- **MIGRATION.md is authoritative** - every successful migration commit must include a refreshed `MIGRATION.md`. If the diff does not contain `MIGRATION.md`, the commit is malformed - block it and invoke `migrate-inventory`.
